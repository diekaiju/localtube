...
public class HtmlRenderer {
    ...
    public static String renderShortsPage(int serviceId, boolean isTv) {
        ...
        sb.append("<style>\n")
          ...
          .append("      '<path d=\"M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.21.05-.42.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z\"/>' :\n")
          .append("      '<path d=\"M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z\"/>';\n")
          .append("    card.innerHTML = `<div class=\"shorts-video-wrapper\" onclick=\"togglePlayCard(this.parentElement)\"><video class=\"shorts-video\" id=\"short-video-${idx}\" onloadedmetadata=\"this.parentElement.style.aspectRatio = this.videoWidth / this.videoHeight\" ontimeupdate=\"const pb = document.getElementById('shorts-progress-bar-${idx}'); if (pb && this.duration) pb.style.width = (this.currentTime / this.duration * 100) + '%';\" loop playsinline preload=\"none\" poster=\"${item.thumbnailUrl || ''}\" muted></video><div class=\"shorts-overlay\"><div class=\"shorts-top-bar\"></div><div class=\"shorts-bottom-info\" onclick=\"event.stopPropagation()\"><div class=\"shorts-author-row\"><div style=\"width:40px;height:40px;border-radius:50%;border:2px solid var(--logo-color, #6750A4);background:${avatarColor};display:flex;align-items:center;justify-content:center;font-weight:700;font-size:16px;color:#fff;flex-shrink:0;\">${firstChar}</div><a href=\"/channel?id=${encodeURIComponent(item.uploaderUrl || '')}\" class=\"shorts-author-name\">${item.uploaderName || 'Creator'}</a><button class=\"shorts-sub-btn\">Subscribe</button></div><div class=\"shorts-title\">${item.name || ''}</div></div><div class=\"shorts-actions\"><div style=\"text-align:center;\" onclick=\"event.stopPropagation()\"><button class=\"shorts-action-btn\" onclick=\"toggleSave(this, '${item.url}', '${item.name || ''}', '${item.uploaderName || ''}', '${item.thumbnailUrl || ''}')\"><svg viewBox=\"0 0 24 24\" fill=\"currentColor\" width=\"24\" height=\"24\"><path d=\"M17 3H7c-1.1 0-1.99.9-1.99 2L5 21l7-3 7 3V5c0-1.1-.9-2-2-2zm0 15l-5-2.18L7 18V5h10v13z\"/></svg></button><div class=\"shorts-action-label\">Save</div></div><div style=\"text-align:center;\" onclick=\"event.stopPropagation()\"><button class=\"shorts-action-btn\" onclick=\"location.href='/watch?id=' + encodeURIComponent('${item.url}')\" title=\"Open in full player\"><svg viewBox=\"0 0 24 24\" fill=\"currentColor\" width=\"24\" height=\"24\"><path d=\"M19 19H5V5h7V3H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2v-7h-2v7zM14 3v2h3.59l-9.83 9.83 1.41 1.41L19 6.41V10h2V3h-7z\"/></svg></button><div class=\"shorts-action-label\">Open</div></div><div style=\"text-align:center;\" onclick=\"event.stopPropagation()\"><button class=\"shorts-action-btn mute-btn\" onclick=\"toggleMuteGlobal(event)\"><svg viewBox=\"0 0 24 24\" fill=\"currentColor\" width=\"24\" height=\"24\">${muteSvg}</svg></button><div class=\"shorts-action-label mute-label\">${isMuted ? 'Muted' : 'Unmuted'}</div></div><div style=\"text-align:center;\" onclick=\"event.stopPropagation()\"><button class=\"shorts-action-btn\" onclick=\"navigator.clipboard.writeText('https://www.youtube.com/watch?v=' + '${item.url}'); alert('Link copied!');\"><svg viewBox=\"0 0 24 24\" fill=\"currentColor\" width=\"24\" height=\"24\"><path d=\"M18 16.08c-.76 0-1.44.3-1.96.77L8.91 12.7c.05-.23.09-.46.09-.7s-.04-.47-.09-.7l7.05-4.11c.54.5 1.25.81 2.04.81 1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3c0 .24.04.47.09.7L8.04 9.81C7.5 9.31 6.79 9 6 9c-1.66 0-3 1.34-3 3s1.34 3 3 3c.79 0 1.5-.31 2.04-.81l7.12 4.16c-.05.21-.08.43-.08.65 0 1.61 1.31 2.92 2.92 2.92 1.61 0 2.92-1.31 2.92-2.92s-1.31-2.92-2.92-2.92z\"/></svg></button><div class=\"shorts-action-label\">Share</div></div></div><div class=\"shorts-progress-bar-container\"><div class=\"shorts-progress-bar\" id=\"shorts-progress-bar-${idx}\"></div></div></div></div>`;\n")
          .append("    container.appendChild(card);\n")
          .append("  }\n")
